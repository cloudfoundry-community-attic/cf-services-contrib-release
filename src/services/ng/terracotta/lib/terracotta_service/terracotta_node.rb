...
require "terracotta_service/util"
...
class VCAP::Services::Terracotta::Node
  ...
  def initialize(options)
    ...
    @terracotta_port = options[:service_port] = 25001
    ...
    # Timeout for terracotta client operations, node cannot be blocked on any terracotta instances.
    # Default value is 2 seconds.
    @terracotta_timeout = options[:terracotta_timeout] || 2
  ...
  def provision(plan = nil, credentials = nil, version = nil)
    ...
    raise TerracottaError.new(TerracottaError::TERRACOTTA_INVALID_PLAN, plan) unless plan.to_s == @plan
  ...
  def bind(name, binding_options = :all, credentials = nil)
    # FIXME: Terracotta has no user level security, just return provisioned credentials.
  ...
  def unbind(credentials)
    # FIXME: Terracotta has no user level security, so has no operation for unbinding.
    {}
  end
  ...
  def restore(instance_id, backup_dir)
    ...
    if File.exists?(dump_file)
      ...
      raise TerracottaError.new(TerracottaError::TERRACOTTA_RESTORE_FILE_NOT_FOUND, dump_file)
    end
    {}
  end
  ...
  def disable_instance(service_credentials, binding_credentials_list = [])
    ...
    set_config(instance.ip, @terracotta_port, instance.password, "requirepass", @disable_password)
    true
  rescue => e
    @logger.warn(e)
    nil
  ...
  def enable_instance(service_credentials, binding_credentials_map = {})
    instance = get_instance(service_credentials["name"])
    set_config(instance.ip, @terracotta_port, @disable_password, "requirepass", instance.password)
    true
  rescue => e
    @logger.warn(e)
    nil
  ...
  def import_instance(service_credentials, binding_credentials_map={}, dump_dir, plan)
    ...
    instance = ProvisionedService.create(
      ...
      db_file)
    instance.run
    raise TerracottaError.new(TerracottaError::TERRACOTTA_START_INSTANCE_TIMEOUT, instance.name) if instance.wait_service_start == false
  ...
  def get_varz(instance)
    info = get_info(instance.ip, @terracotta_port, instance.password)
    varz = {}
  ...
  def get_instance(name)
    instance = ProvisionedService.get(name)
    raise TerracottaError.new(TerracottaError::TERRACOTTA_FIND_INSTANCE_FAILED, name) if instance.nil?
    instance
  end
...
class VCAP::Services::Terracotta::Node::ProvisionedService
  ...
  class << self
    ...
    def create(port, version, plan=nil, name=nil, password=nil, db_file=nil, is_upgraded=false)
      ...
      log_file = File.join(instance.log_dir, "terracotta.log")
  ...
  def finish_start?
    terracotta = Terracotta.new({:host => ip, :port => @@options[:service_port], :password => password})
    terracotta.echo("")
    true
  rescue => e
    false
  ...
  def migration_check
    super
    # Need regenerate configuration file for terracotta server
    VCAP::Services::Terracotta::Node::ProvisionedService.create(port, version, plan, name, password, nil, true) if container.nil?
  end
end
