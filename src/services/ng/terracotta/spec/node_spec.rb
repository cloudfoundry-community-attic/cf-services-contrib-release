...
describe VCAP::Services::Terracotta::Node do
  ...
  before :all do
    ...
    EM.run do
      @node = VCAP::Services::Terracotta::Node.new(@options)
      EM.add_timer(1) {EM.stop}
    end
    @terracotta_port = @node.options[:service_port]
  end
  ...
  describe "Node.pre_send_announcement" do
    ...
    it "should start provisioned instances before sending announcement" do
      ...
      terracotta_echo(instance.ip, @terracotta_port, @credentials["password"]).should == true
    end
  ...
  describe "Node.provision" do
    ...
    it "should not allow null credentials to access the instance" do
      expect {terracotta_echo(@instance.ip, @terracotta_port)}.should raise_error(RuntimeError)
    end

    it "should not allow wrong credentials to access the instance" do
      expect {terracotta_echo(@instance.ip, @terracotta_port, "wrong_password")}.should raise_error(RuntimeError)
    end
  ...
  describe "Node.unprovision" do
    ...
    it "should not access the instance after unprovision" do
      expect {terracotta_echo(@instance.ip, @terracotta_port, @credentials["password"])}.should raise_error
    end
  ...
  describe "Node.bind" do
    ...
    it "should access terracotta server using the returned credentials" do
      terracotta_echo(@instance.ip, @terracotta_port, @binding_credentials["password"]).should == true
    end
  ...
  describe "Node.restore" do
    ...
    it "should restore user data from a backup file" do
      terracotta_get(@instance.ip, @terracotta_port, @credentials["password"], "test_key").should == "test_value"
    end
    ...
    it "should restore from an empty dump file" do
      restore_dir = "/tmp/terracotta_empty"
      ...
      restore_result.should == {}
      @instance = @node.get_instance(@credentials["name"])
      terracotta_get(@instance.ip, @terracotta_port, @credentials["password"], "test_key").should be_nil
      FileUtils.rm_rf(restore_dir)
    end

    it "should raise exception if the instance is not existed" do
      expect {@node.restore("non-existed", "/tmp")}.should raise_error(VCAP::Services::Terracotta::TerracottaError)
    end

    it "should raise exception if the dumped file is not existed" do
      expect {@node.restore(@credentials["name"], "/tmp/restore")}.should raise_error(VCAP::Services::Terracotta::TerracottaError)
    end
  end
  ...
  describe "Node.migration" do
    before :all do
      @credentials = @node.provision(:free, nil, @default_version)
      @instance = @node.get_instance(@credentials["name"])
      terracotta_set(@instance.ip, @terracotta_port, @credentials["password"], "test_key", "test_value")
      @dump_dir = File.join("/tmp/migration/terracotta", @credentials["name"])
    ...
    it "should not access terracotta server after disable the instance" do
      @node.disable_instance(@credentials, @binding_credentials_list).should == true
      expect {terracotta_echo(@instance.ip, @terracotta_port, @credentials["password"])}.should raise_error(RuntimeError)
    end
    ...
    it "should access terracotta server in old node after enable the instance" do
    ...
    it "should import db file from right location after import instance" do
      ...
      terracotta_get(instance.ip, @terracotta_port, credentials_list[0]["password"], "test_key").should == "test_value"
      credentials_list[1].each do |_, value|
        terracotta_get(instance.ip, @terracotta_port, value["credentials"]["password"], "test_key").should == "test_value"
      end
    end
  end
    ...
    it "should limit the maximum number of clients" do
      ...
      begin
        ...
        for i in 1..@options[:max_clients]
          terracotta[i] = Terracotta.new({:host => instance.ip, :port => @terracotta_port, :password => credentials["password"]})
          terracotta[i].info
        end
        # The max_clients + 1 connection will raise exception
        expect {Terracotta.new({:host => instance.ip, :port => @terracotta_port, :password => credentials["password"]}).info}.should raise_error(RuntimeError)
        # Close the max_clients connections
        for i in 1..@options[:max_clients]
          terracotta[i].quit
        end
        # Now the new connection will be successful
        terracotta = Terracotta.new({:host => instance.ip, :port => @terracotta_port, :password => credentials["password"]})
        terracotta.info.should be
  ...
  describe "Node.timeout" do
    it "should raise exception when terracotta client response time is too long" do
      credentials = @node.provision(:free, nil, @default_version)
      instance = @node.get_instance(credentials["name"])
      class Terracotta
        alias :old_info :info
      ...
      expect {@node.get_info(instance.ip, @terracotta_port, credentials["password"])}.should raise_error(VCAP::Services::Terracotta::TerracottaError)
      class Terracotta
        alias :info :old_info
      end
      @node.get_info(instance.ip, @terracotta_port, credentials["password"]).should be
      @node.unprovision(credentials["name"])
    end
  end
  ...
  describe "Node.thread_safe" do
    it "should be thread safe in multi-threads call" do
      ...
      VCAP::Services::Terracotta::Node::ProvisionedService.all.size.should == threads_num
      ...
      VCAP::Services::Terracotta::Node::ProvisionedService.all.size.should == 0
    end
  end
