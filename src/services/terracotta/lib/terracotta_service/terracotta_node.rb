...
class VCAP::Services::Terracotta::Node
  ...
  def initialize(options)
    ...
    @terracotta_log_dir = options[:terracotta_log_dir]
    @terracotta_path = options[:terracotta_path]
    @terracotta_plugin_dir = options[:terracotta_plugin_dir]
    @logger.debug(options[:terracotta_plugin_dir])
  ...
  def varz_details
    ...
    # Get terracotta health, index & process status
    stats = []
    ProvisionedService.all.each do |provisioned_service|
      stat = {}
      stat['health'] = terracotta_health_stats(provisioned_service)
      stat['index'] = terracotta_index_stats(provisioned_service)
      stat['process'] = terracotta_process_stats(provisioned_service)
    ...
    begin
      ProvisionedService.all.each do |instance|
        provisioned_instances[instance.name.to_sym] = terracotta_status(instance)
      end
    rescue => e
  ...
  def provision(plan, credentials = nil, version=nil)
    ...
    if credentials
      ...
      provisioned_service.name = "terracotta-#{UUIDTools::UUID.random_create.to_s}"
  ...
  # So "http-basic(https://github.com/Asquera/terracotta-http-basic)" plugin
  ...
  def terracotta_health_stats(instance)
    ...
    warning = "Failed terracotta_health_stats: #{e.message}, instance: #{instance.name}"
    @logger.warn(warning)
    warning
  end

  def terracotta_index_stats(instance)
    ...
    warning = "Failed terracotta_index_stats: #{e.message}, instance: #{instance.name}"
    @logger.warn(warning)
    warning
  end

  def terracotta_process_stats(instance)
    ...
    warning = "Failed terracotta_process_stats: #{e.message}, instance: #{instance.name}"
    @logger.warn(warning)
    warning
  end

  def terracotta_status(instance)
  ...
  def setup_server(dir, provisioned_service)
    @logger.info("Installing terracotta to #{dir}")
    ...
    plugins_dir = @terracotta_plugin_dir
  ...
  def config_path(dir)
    File.join(config_dir(dir), 'terracotta.yml')
  end
  ...
  def log_dir(instance_id)
    File.join(@terracotta_log_dir, instance_id)
  end
end
